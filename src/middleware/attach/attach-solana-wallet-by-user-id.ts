...
export default async function attachSolanaWalletByUserId(req: Request, res: Response, next: NextFunction) : Promise<void | Response> {
	try {
		const user = req.user
		const solanaWallet = await findSolanaWalletByUserId(user.user_id)
		if (_.isNull(solanaWallet)) {
			return res.status(400).json({ message: "Cannot find Solana Wallet" })
		}
