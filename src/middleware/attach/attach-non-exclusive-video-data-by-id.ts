...
import retrieveNonExclusiveVideoById from "../../db-operations/read/video/retrieve-non-exclusive-video-by-id"
...
export default async function attachNonExclusiveVideoDataById(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
	try {
		...
		const { user } = req
		const nonExclusiveVideoData = await retrieveNonExclusiveVideoById(Number(videoId), user.user_id)
