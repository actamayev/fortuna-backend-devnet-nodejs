...
export default async function uploadThumbnailPicture (req: Request, res: Response): Promise<Response> {
	try {
		...
		const uploadThumbnailToS3Key = createS3Key("uploaded-images", uuid)
		const imageUploadUrl = await AwsS3.getInstance().uploadImage(buffer, uploadThumbnailToS3Key)
